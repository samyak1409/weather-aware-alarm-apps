...
- The body is N3's plus ` · watching until {checktime}` — and that deadline phrase matches N17's planned-cap half (same `fmtCheckTime` vs the alarm). **`{checktime}` here is this alarm's retry cap** (T+1 / T+30 / T+60 — see N13). Pulled from the shade on delete, court-gone, or toggle-off / abandon-edit while still an open promise; after a final, toggle-off leaves it. N3 always survives a toggle-off.
- **Keep checking mid-window (2026-07-26):** only the deadline may move — same notification id, quiet update (Android `onlyAlertOnce`; iOS `presentSound: false` — quiets foreground **and** background per plugin docs — + `presentBanner: false`; `presentAlert` deliberately unset — iOS 10–14 only, inert at our floor of 26; `presentList` left unset so `defaultPresentList: true` keeps the card in Notification Center). Wind numbers and limits stay whatever the first heads-up said (never rebuild from the edited alarm — that would read "Too windy · wind 5 (≤20)"). Shrink-past-now updates the deadline and **keeps** N2 so N3 can post beside it.
- **Left standing after the outcome — locked 2026-07-22, don't "fix" it.** Nothing cancels this card after a late ring or final skip, so after a late ring at 06:07 it still reads `Still checking … watching until 06:30` (or the Keep-checking-updated deadline). Accepted: its counterpart arrives as its own card (N3) or as the ring itself (N1), and history keeps both moments as separate rows. Note the asymmetry is real — N17 ages `watching` → `watched` once a final lands; the notification body does not.
...
History is an **append-only log of events mirroring the notifications** (2026-07-20): an occurrence that misses its T leaves the heads-up **snapshot row** right then (what N2 said, marked with N17's watch note), and its **final outcome** — the cap's skip, or a late ring — is a **separate second row**. Both stay forever; new events never overwrite other events. Snapshot **annotations** (same row, 2026-07-26): Keep checking may move `watchedUntil` only; user kill stamps `watchStoppedAt` — wind numbers/limits stay decision-time.
...
- Final landed (late ring / cap skip): ` · watched until {checktime}` → ` · watched until 06:30`
- User killed the watch early (toggle-off, abandon edit, delete alarm, …): ` · watching until {checktime} · stopped at {checktime}` → ` · watching until 06:30 · stopped at 06:05` — keeps the planned cap (what the window was set for) and appends when checking stopped (`watchStoppedAt`)
- Past the cap, no final, not user-stopped: ` · watching until {checktime} · failed` → ` · watching until 06:30 · failed`
- `{checktime}` for the first half is the snapshot's stored `watchedUntil` (= that alarm's retry cap; may move if Keep checking is widened/shrunk mid-window). Appended to the sheet sub. Final rows never carry it.
