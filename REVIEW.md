...
**1.4 — Notification permission requested on BOTH platforms at first launch.** Android 13+ needs the runtime grant; iOS needs it too, because the `alarm` package only _checks_ iOS permission and never asks. Without it, Nivaat's skip cards / Arunoday's bedtime banner would silently never appear on iOS. _Example:_ first open → OS permission prompt.
...
**1.7 — On resume, both apps re-sync** (reload from storage + recompute + reschedule), since a background check or notification action may have changed state.

**1.8 — Theme:** pure-black OLED, dark-only, minimal. Arunoday = dawn orange, Nivaat = sky blue.
...
**2.10 — Polar refusal.** A location with no daily civil dawn (high latitude, part of the year) is refused on add _and_ on activate; if an active location ever loses its dawn, home shows a dedicated "no dawn here" screen. _Example:_ a high-Arctic spot → rejected.
...
**3.8 — History.** Every ring and skip is logged with the wind that caused it and the **limits in force at that time** (so an old row still reads correctly after you change a limit). A row is anchored to the alarm's time and **always** shows the **wind-check time** behind it — "· checked HH:MM" (last successful reading; e.g. a 06:00 ring on a 22:00 check), or "· last tried HH:MM" for a no-data skip (its last attempt, since nothing was read) — as reinforcement the result came from a real check. _Example:_ "Skipped (gusty) · wind 3 (≤4) · gusts 16 (≤15) km/h".

**3.9 — A ring is never silenced by opening the app.** If a ring is sounding and you open the app, it keeps ringing (never cancelled or relabelled). A ring that already fired is recorded as "rang" even if you open the app much later, or the wind has since risen. _Example:_ rings 06:00 on a calm forecast; you open the app at 06:50 in a fresh gust → history says **rang**, not skipped.
