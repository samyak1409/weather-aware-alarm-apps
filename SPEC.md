...
- **Bedtime ritual (v1)**: the bedtime ring screen shows tomorrow's wake time and offers: "not sleepy" → delay bedtime +30m/+1h (re-rings, id 2999); "tomorrow +1h/+2h" → one-time wake offset applied only to the next wake, auto-cleared after it passes. Rationale: Samyak opens the app nightly anyway — the bedtime ring is the natural checkpoint to confirm/adjust tomorrow.
...
- **Fail-safe (user decision)**: windy → no ring; API/network fail → no ring. Every skip — windy, gusty, AND no-data — leaves a **silent notification + in-app history entry** ("Skipped: wind 9 km/h at 6:00" / "Could not check the wind — alarm skipped"). Implemented in v1 (flutter_local_notifications). Trust through honesty, never silent failure.
